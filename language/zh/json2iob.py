...
if answer == 'y':
    ...
    tokenizer = getattr(importlib.import_module(f'spacy.lang.{code}'), language)
    nlp = tokenizer()
...
def convert(content, annotations):
    ...
    indices = sorted(flatten([[annotation['start'], annotation['end']] for annotation in annotations]))
    splitted_content = [(i, j, content[i:j]) for i,j in zip(indices, indices[1:]+[None])]
...
for dataset, destination in zip([train_set, dev_set, test_set], ['train', 'dev', 'test']):
    result = '\n\n'.join([convert(example['content'], example['annotations']) for example in dataset])
...
with open(os.path.join(output_dir, f'labels.txt'), 'w') as f:
    f.write('\n'.join(sorted(list(set([label.replace(' ','') for label in label_set])), key=lambda x: x.split('-')[1] if x != 'O' else x)))